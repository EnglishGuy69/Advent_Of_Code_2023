...
class ChunkCombination:
    def __init__(self, s:str):
        self.s=s
        self.segments = [seg for seg in s.split('.') if seg != '']


class Chunk:
    def __init__(self, chunk_str: str):
        num_damaged = len([c for c in chunk_str if c == SpringData.DAMAGED])
        num_undefined = len(chunk_str) - num_damaged
        for i in range(0, 2**num_undefined):
            data = self.data
            bin_number = bin(i)[2:]
...
class SpringData:
    ...
    def __init__(self, data: str, damage_summary:[int], debug=False):
        ...
        # for chunk in chunks:
        #     self.chunks.append(ChunkRange(chunk))
    ...
    def __init__(self, filename, debug=False):
        ...
        with open(filename, 'r') as f:
            for row in f.readlines():
                ...
                spring_data = '?'.join([spring_data,spring_data,spring_data,spring_data,spring_data])
                summary_txt = row.strip().split()[1].split(',')
                summary_txt = summary_txt * 5
                summary = [int(n) for n in summary_txt]
